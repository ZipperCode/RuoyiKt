...
object ExcelUtil {
    ...
    fun <T> HttpServletResponse.writeToExcel(
        data: List<T>, sheetName: String,
        clazz: Class<T>,
        merge: Boolean = false,
        ...
        try {
            ...
            outputStream.writeToExcel(data, sheetName, clazz, merge, writeHandler)
        } catch (e: IOException) {
     ...
     * @param merge     是否合并单元格
     */
    fun <T> OutputStream.writeToExcel(
        data: List<T>,
        sheetName: String,
        clazz: Class<T>,
        merge: Boolean = false,
        ...
        val builder = EasyExcel.write(this, clazz)
        ...
        if (merge) {
            // 合并处理器
            builder.registerWriteHandler(CellMergeStrategy(data, true))
        }
        // 添加下拉框操作
    ...
    fun resetResponse(sheetName: String, response: HttpServletResponse) {
        val filename = encodingFilename(sheetName)
        response.setAttachmentResponseHeader(filename)
    ...
    fun convertByExp(propertyValue: String, converterExp: String, separator: String): String {
        ...
        val convertSource: Array<String> = converterExp
            .split(Regex(","))
    ...
    fun reverseByExp(propertyValue: String, converterExp: String, separator: String): String {
        val propertyString = StringBuilder()
        val convertSource: Array<String> = converterExp.split(Regex(",")).dropLastWhile { it.isEmpty() }.toTypedArray()
        for (item in convertSource) {
            val itemArray = item.split("=".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            if (StringUtils.containsAny(propertyValue, separator)) {
                for (value in propertyValue.split(separator.toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()) {
