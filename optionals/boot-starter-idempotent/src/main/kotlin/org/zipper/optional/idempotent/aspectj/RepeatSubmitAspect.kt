package org.zipper.optional.idempotent.aspectj
...
class RepeatSubmitAspect {
    ...
    fun doBefore(point: JoinPoint, repeatSubmit: RepeatSubmit) {
        ...
        val request = Thread.currentThread().getRequest() ?: throw ServiceException("获取当前请求为空")
        ...
        // 唯一值（没有消息头则使用请求地址）
        var submitKey: String = StringUtils.trimToEmpty(
            request.getHeader(SaManager.getConfig().tokenName)
        )
