package org.zipper.modules.system.service.user
...
class SysUserServiceImpl(
    ...
    override fun selectUserRoleGroup(userName: String?): String? {
        ...
        return list.mapNotNull { it.roleId }.joinToString(",")
    }
    ...
    override fun selectUserPostGroup(userName: String?): String? {
        ...
        return list.joinToString(",") { it.postName ?: "" }
    }
    ...
    private fun insertUserPost(user: SysUserParam, clear: Boolean) {
        ...
        if (ArrayUtil.isNotEmpty<Long>(posts)) {
            ...
            // 新增用户与岗位管理
            val list: List<SysUserPostEntity> = posts.map { postId ->
                SysUserPostEntity().apply {
                    userId = user.userId
                    this.postId = postId
                }
            }
    ...
    private fun insertUserRole(userId: Long?, roleIds: Array<Long>, clear: Boolean) {
        if (ArrayUtil.isNotEmpty<Long>(roleIds)) {
            ...
            val canDoRoleList: List<Long> = roleIds.filter { roleList.contains(it) }
            ...
            val list: List<SysUserRoleEntity> = canDoRoleList.map {
                SysUserRoleEntity().apply {
                    this.userId = userId
                    this.roleId = it
                }
            }
