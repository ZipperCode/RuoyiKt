package org.zipper.modules.system.service.role
...
import org.zipper.modules.system.mapper.SysUserRoleMapper
import java.util.*
...
class SysRoleServiceImpl(
    ...
    override fun selectRolePermissionByUserId(userId: Long): Set<String> {
        ...
        for (perm in perms) {
            if (ObjectUtil.isNotNull(perm)) {
                perm.roleKey?.trim { it < ' ' }?.let {
                    permsSet.addAll(it.split(","))
                }
            }
        }
        return permsSet
    ...
    override fun cleanOnlineUserByRole(roleId: Long?) {
        ...
        val keys = StpUtil.searchTokenValue("", 0, -1, false) ?: return
