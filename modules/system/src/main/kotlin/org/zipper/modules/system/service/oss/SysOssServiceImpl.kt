package org.zipper.modules.system.service.oss
...
import org.zipper.common.core.ext.setAttachmentResponseHeader
...
import org.zipper.framework.oss.entity.UploadResult
import org.zipper.framework.oss.enumd.AccessPolicyType
import com.zipper.framework.oss.factory.OssFactory
import org.zipper.modules.system.domain.bo.SysOssBo
import org.zipper.modules.system.domain.entity.SysOssEntity
import org.zipper.modules.system.domain.vo.SysOssVo
import org.zipper.modules.system.mapper.SysOssMapper
import java.io.File
...
class SysOssServiceImpl(
    ...
    override fun queryPageList(sysOss: SysOssBo, pageQuery: PageQuery): TableDataInfo<SysOssVo> {
        ...
        val result = baseMapper.selectVoPage<SysOssEntity, SysOssVo>(pageQuery.build(), lqw)
    ...
    override fun listByIds(ossIds: Collection<Long>): List<SysOssVo> {
        ...
        for (id in ossIds) {
            val vo: SysOssVo? = selfAopProxy().getById(id)
            if (vo != null) {
                try {
    ...
    override fun selectUrlByIds(ossIds: String?): String? {
        if (ossIds.isNullOrEmpty()) {
            return null
        }
        val list: MutableList<String?> = ArrayList()
        ossIds.split(",").forEach { id ->
            val vo = selfAopProxy().getById(id.toLong())
            if (vo != null) {
                try {
    ...
    override fun getById(ossId: Long?): SysOssVo? {
        return baseMapper.selectById(ossId).convertOrNull()
    }

    @Throws(IOException::class)
    override fun download(ossId: Long?, response: HttpServletResponse) {
        val sysOss = selfAopProxy().getById(ossId) ?: throw ServiceException("文件数据不存在!")
        response.setAttachmentResponseHeader(sysOss.originalName ?: "")
    ...
    private fun buildResultEntity(originFilename: String, suffix: String, configKey: String, uploadResult: UploadResult): SysOssVo {
        ...
        val sysOssVo = oss.convert<SysOssVo>()
