...
const handleFileSuccess: UploadProps['onSuccess'] = (res: any): void => {
  ...
  console.log("上传文件成功，resp = ", res)
  // 删除自身
  const respData = res.data as FileRecordVo
  const fileKey = respData.hash || respData.id.toString()

  const index = fileList.value.findIndex((item) => item.name === fileKey)
  fileList.value.splice(index, 1)
  uploadList.value.push({ name: fileKey, url: respData.url })
